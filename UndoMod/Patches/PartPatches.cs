...
namespace UndoMod
{
    ...
    static class Patch_Delete { static void Postfix() => SnapHelper.DoStructural(); }

    [HarmonyPatch(typeof(CEManager), nameof(CEManager.DuplicatePart))]
    static class Patch_Duplicate { static void Postfix() => SnapHelper.DoStructural(); }

    [HarmonyPatch(typeof(CEManager), nameof(CEManager.PlaceParts))]
    static class Patch_Place { static void Postfix() => SnapHelper.DoStructural(); }
    ...
    static class Patch_Mirror { static void Postfix() => SnapHelper.DoStructural(); }

    [HarmonyPatch(typeof(CEManager), nameof(CEManager.ToggleSymmetry))]
    static class Patch_Symmetry { static void Postfix() => SnapHelper.DoStructural(); }

    [HarmonyPatch(typeof(CEManager), nameof(CEManager.ImportMesh))]
    static class Patch_Import { static void Postfix() => SnapHelper.DoStructural(); }
